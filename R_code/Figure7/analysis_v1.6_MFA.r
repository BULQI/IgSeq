...
#setwd("E:/feng/LAB/hg/IgSeq_MS/manuscript/clustering/")
...
ggplot(data=temp)+
  ...
  plot3d( xlab="PC1", ylab="PC2",zlab="PC3",x=d.quali[,1],y=d.quali[,2], z=d.quali[,3],
    ...
    #legend, OVA +Alum: circle; OVA : square;  diamond : PBS ; triangle: OVA+CpG
        #
        pch3d( xlab="PC1", ylab="PC2",zlab="PC3",x=d.quali[,1],y=d.quali[,2], z=d.quali[,3],
    ...
    play3d(spin3d( axis=c(0,0,1),rpm=4), duration=10)
        movie3d(spin3d( rpm=3), duration=10)
...
#plot 3d to set scale and limits
 plot3d( xlab="PC1", ylab="PC2",zlab="PC3",x=dd[,1],y=dd[,2], z=dd[,3],
                    col=1, 
...
dd.iso[dd.iso$treatment=="OVA+Alum","pch"]<-17
 ...
 pch3d( xlab="PC1", ylab="PC2",zlab="PC3",x=dd.iso[,1],y=dd.iso[,2], z=dd.iso[,3],
...
#axes3d()  <-not necessary since we calling plot3d to set up the canvas
    ...
    play3d(spin3d( rpm=4), duration=10)
        movie3d(spin3d( rpm=3), duration=10)
        png(file="MFA_movie3d_ind_legend.png", width=600, height=600)
plot(x=c(0,1),y=c(0,1), type="n", yaxt="n", xaxt="n", xlab="", ylab="")
...
bars$tissue<-cols_2[bars$tissue]
 ...
 setwd("/home/feng/Windows/windowsD/feng/LAB/hg/IgSeq_MS/manuscript/clustering/pc3/")
 png("hiearchicalCluster.png", width=1000, height=400)
...
save(#dx, bars,
                            d.all.conds, #cols_4, 
                            dd, d.out, file=("MFA.hclust.plots.v1.6_pc3.rData"))
